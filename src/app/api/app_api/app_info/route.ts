...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        //   );
        // }
        const { platform, auth_token} = await request.json();
        // const fdata = {
...
//  here we get the customer company info if auth_token is passed in api
        ...
        if(auth_token){
            const {data,error} = await supabase
            .from("leap_customer")
            .select("auth_token, customer_id, leap_client(leap_client_basic_info(*))")
        ...
        const {data:DashboardPermission,error:perError} = await supabase.from("leap_client_employee_permissions")
            ...
            .eq("customer_id",companyDetails[0].customer_id)
...
// here we create the result api response data into a single object
        ...
        if (auth_token) {
            ...
            appInfoResult = {
                ...
                customer_id: companyDetails[0]?.customer_id,
