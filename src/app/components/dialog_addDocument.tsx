...
import SampleTemplateView from './dialog_sampleTemplateView';
...
const DialogUploadDocument = ({ onClose, docType }: { onClose: () => void, docType: any }) => {
    ...
    const [isTypeTemplate, setIsTypeTemplate] = useState(false);
    const [showHtmlGenDialog, setShowHtmlGeneratedDialog] = useState(false);
    ...
    const generateHtmlFromDoc = async (file: File) => {
        setLoading(true);
        ...
        try {
            ...
            const res = await fetch("/api/clientAdmin/document-html", {
                method: "POST",
                body: formData,
            });
            ...
            if (data.status == 1) {
                ...
                setLoading(false);
                setHtml(data.data);
                setShowHtmlGeneratedDialog(true);
    ...
    const handleDocTypeChange = (docTypeID: any) => {
        ...
        for (let i = 0; i < docTypes.length; i++) {
            if (docTypes[i].id == docTypeID && docTypes[i].document_type_id == 6) {
                setIsTypeTemplate(true);
                break;
            } else {
                setIsTypeTemplate(false);
            }
        }
        ...
        return (
            ...
            <div>
                ...
                <div className="row">
                    ...
                    {!isTypeTemplate && <div className="col-lg-12 mb-5">
                        <div className="col-lg-12">
